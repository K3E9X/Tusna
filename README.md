...
- **Maigret collector** (optional worker in [`collector/`](collector/)): for real depth, Tusna delegates to **Maigret** — 3000+ sites *with profile-data extraction and identifier discovery* — and pulls the structured result into the same graph. Deploy the worker (Render/Railway/Fly, free) and set `COLLECTOR_URL` on Vercel; the "Maigret (deep)" app then feeds every scan. Without it, the built-in connectors are used. This is the "let a proven tool do the heavy collection, we pull and correlate" model.
