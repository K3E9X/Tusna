...
**Recursive pivot.** Select any discovered node (an email, an alias, a platform) and hit **⌖ PIVOT ON THIS**: Tusna rescans that identifier and **merges the new hop into the same board**, linked to the node you pivoted from — so one seed grows outward into a real constellation, hop by hop (like Maigret's recursion, but on your graph, under your control). A pivoted account that's genuinely the same as one already on the board reuses its node; a different person's same-platform account is namespaced so it never collides.
